...
case class AssignationHelper() extends InterpreterHelper {
  override def interpret(
    ...
    if (variables.check(name)) {
      ...
      val result =
        ExpressionCalculator(variables, constants).calculate(ast.nodes(1))
      variables.add(name, result.value, dataType)
    } else if (constants.check(name)) {
      throw new Exception(
        s"Reassigning constant at line ${lexicalRange.getStartLine}, column ${lexicalRange.getStartCol}"
      )
    } else {
      throw new Exception(
        s"Non existing identifier at line ${lexicalRange.getStartLine}, column ${lexicalRange.getStartCol}"
      )
    }
  }
}
