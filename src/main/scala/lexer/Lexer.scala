...
class Lexer {
  def getTokens(iterator: Iterator[Char]): List[Token] = {
    ...
    while (iterator.hasNext) {
      ...
      if (newToken.getType == Newline) {
        currentLexicalRange = new LexicalRange(1, currentLexicalRange.getStartLine + 1, 1, currentLexicalRange.getEndLine + 1)
      } else {
        currentLexicalRange = new LexicalRange(newTokenLexicalRange.getEndCol + 1, newTokenLexicalRange.getStartLine, newTokenLexicalRange.getEndCol + 1, newTokenLexicalRange.getEndLine)
      }
    }
    tokens += new Token(EndOfFile, currentIndex, currentIndex, currentLexicalRange)
    tokens.toList
  }
  ...
  private def getToken(currentIndex: Int, currentLexicalRange: LexicalRange, iterator: Iterator[Char]): Token = {
    popCharacter(iterator) match {
      case None => new Token(EndOfFile, currentIndex-1, currentIndex-1, currentLexicalRange)
      case Some(char) => char match {
        ...
        case '(' => new Token(OpenParenthesis, currentIndex, currentIndex + 1, currentLexicalRange)
        case ')' => new Token(ClosedParenthesis, currentIndex, currentIndex + 1, currentLexicalRange)
        case '-' => new Token(Minus, currentIndex, currentIndex + 1, currentLexicalRange)
        case '+' => new Token(Plus, currentIndex, currentIndex + 1, currentLexicalRange)
        case '*' => new Token(Asterisk, currentIndex, currentIndex + 1, currentLexicalRange)
        case '/' => new Token(FrontSlash, currentIndex, currentIndex + 1, currentLexicalRange)
        case '\'' | '"' => processString(char.toString, currentIndex, currentIndex + 1, currentLexicalRange, iterator)
        case '=' => new Token(Assignment, currentIndex, currentIndex + 1, currentLexicalRange)
        case ':' => new Token(Colon, currentIndex, currentIndex + 1, currentLexicalRange)
        case ';' => new Token(Semicolon, currentIndex, currentIndex + 1, currentLexicalRange)
        case '\n' => new Token(Newline, currentIndex, currentIndex + 1, currentLexicalRange)
        case ' ' => getToken(
          currentIndex + 1,
          new LexicalRange(currentLexicalRange.getStartCol + 1, currentLexicalRange.getStartLine, currentLexicalRange.getEndCol + 1, currentLexicalRange.getEndLine),
          iterator
        )
      }
    }
    ...
    readCharacter(iterator) match {
      ...
      case Some(char) => char match {
        case char if Lexer.isDigit(char) | char == '.' => iterator.next; processNumber(
          ...
          from,
          to + 1,
          new LexicalRange(lexicalRange.getStartCol, lexicalRange.getStartLine, lexicalRange.getEndCol + 1, lexicalRange.getEndLine),
          iterator)
        case _  => new Token(token.types.NumberValue, from, to + 1, lexicalRange)
      }
    }
  }
  ...
  private def processString(currentString: String, from: Int, to: Int, lexicalRange: LexicalRange, iterator: Iterator[Char]): Token = {
    ...
    popCharacter(iterator) match {
      ...
      case Some(char) => char match {
        case char if char == initialQuote => new Token(token.types.StringValue, from, to + 1, new LexicalRange(lexicalRange.getStartCol, lexicalRange.getStartLine, lexicalRange.getEndCol + 1, lexicalRange.getEndLine))
  ...
  private def processIdentifier(currentValue: String, from: Int, to: Int, lexicalRange: LexicalRange, iterator: Iterator[Char]): Token = {
    readCharacter(iterator) match {
      case None => currentValue match { // no se que tan necesaria es esta parte
      ...
      case Some(char) => char match {
        case char if char.toString matches "[_0-9a-zA-Z]" => iterator.next; processIdentifier(
          ...
          new LexicalRange(lexicalRange.getStartCol, lexicalRange.getStartLine, lexicalRange.getEndCol + 1, lexicalRange.getEndLine),
          iterator)
        case char if currentValue == "let" && char.isWhitespace => new Token(Let, from, to + 1, lexicalRange)
        case char if currentValue == "println" && (char.toString matches "[ (]") => new Token(Println, from, to + 1, lexicalRange)
