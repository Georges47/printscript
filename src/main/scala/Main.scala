...
object Main extends App {
  ...
  val fileContentAsString = bufferedSource.mkString
  bufferedSource.close

  val lexer = new Lexer
  val tokens = lexer.getTokens(fileContentAsString.toIterator)
  ...
  val ast = parser.parse(fileContentAsString, tokens)
